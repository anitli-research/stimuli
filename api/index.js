...
app.get("/api/auth", basicAuth, async (req, res) => {
  res.sendStatus(200);
});
...
app.get("/api/pool/:poolId", upload.none(), async (req, res) => {
...
app.get("/api/pool/:poolId/:stimulusId", upload.none(), async (req, res) => {
...
app.post("/api/pool/", upload.none(), async (req, res) => {
...
app.post("/api/pool/:poolId", upload.array("stimuli"), (req, res) => {
...
app.post("/api/experiment/", upload.none(), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const q_exp = 'INSERT INTO experiments(`name`, `pool_id`, `accuracy`) VALUES (?, ?, ?)';
      const vals_exp = [req.body.name, req.body.pool_id, req.body.accuracy];
      try {
        ...
        const block_ids = blocks.map(async (block, block_idx) => {
          ...
          for (const rel_k in block.rel) {
            for (const stId_idx in block.rel[rel_k]) {
              const stId = block.rel[rel_k][stId_idx];
...
app.post("/api/experiment/:experimentName", upload.none(), async (req, res) => {
  const exp_name = req.params.experimentName;
  console.log(`POST /experiment/ update experiment  ${exp_name}`);
  let rel, blocks;
...
app.get("/api/experiment/:experimentName", upload.none(), async (req, res) => {
  // const is_id = req.body.is_id !== undefined;
  // console.log(`GET /experiment/${req.params.experimentName} Get experiment by ${is_id ? "id" : "name"}`);
  console.log(`GET /experiment/${req.params.experimentName} Get experiment by name`);
  // const q = `SELECT * FROM experiments WHERE ${is_id ? "experiment_id" : "name"} = ?;`
...
// Session
app.get("/api/session/", upload.none(), async (req, res) => {
...
app.post("/api/session/", upload.none(), async (req, res) => {
  ...
  const block_ids = blocks.map(e => e.block_id);

  const q_rel = `SELECT * FROM relations WHERE block_id in (${block_ids.map(() => '?').join(',')});`;

  const [rels, fields_rel] = await dbpool.query(q_rel, block_ids).catch(e => {
    console.log(e);
  ...
  for (let i = 0; i < rels.length; i++) {
    ...
    relMap[rels[i].block_id][rels[i].l].push(rels[i].r);
  }
  ...
  for (let block_idx = 0; block_idx < blocks.length; block_idx++) {
    ...
    for (let trial_idx = 0; trial_idx < block.n_trials; trial_idx++) {
      ...
      switch (block.type) {
        case 0: {
          // Relation
          const l_idx = Math.floor(Math.random() * rel_keys.length);
          stimulus = rel_keys[l_idx];
          const r_idx = Math.floor(Math.random() * rel[stimulus].length);
          expected = rel[stimulus][r_idx];
          break;
        }
        case 1: {
          // "Reflexive"
        ...
        case 2: {
          // symmetric
          console.log(rel_keys);
          const l_idx = Math.floor(Math.random() * rel_keys.length);
          expected = rel_keys[l_idx];
          const r_idx = Math.floor(Math.random() * rel[expected].length);
          stimulus = rel[expected][r_idx];
          break;
        }
        case 3: {
          // transitive
          const l_idx = Math.floor(Math.random() * rel_keys.length);
          stimulus = rel_keys[l_idx];
          const i_idx = Math.floor(Math.random() * rel[stimulus].length);
          const i = rel[stimulus][i_idx];
          const r_idx = Math.floor(Math.random() * rel[i].length);
          expected = rel[i][r_idx];
          break;
        }
      }
...
app.post("/api/session/:sessionId", upload.none(), async (req, res) => {
...
app.get("/api/block/:experimentId", upload.none(), async (req, res) => {
...
// csv
app.get("/api/session/users", upload.none(), async (req, res) => {
...
app.get("/api/csv/:username", upload.none(), async (req, res) => {
...
// Response
app.get("/api/response/:session_id", upload.none(), async (req, res) => {
...
app.post("/api/response", upload.none(), async (req, res) => {
